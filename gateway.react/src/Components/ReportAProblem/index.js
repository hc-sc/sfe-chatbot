          ...
          return(
            <Fragment>
              <p className="well well-sm text-center mrgn-tp-lg">
                { ( loading || error ) ? null : <ReactMarkdown escapeHtml={ false } source={ data.reportApRoblem[ `alert_immediate_assist_${ locale }` ] } renderers={{
                  paragraph: props =>
                    <span { ...props } />
                }} /> }
              </p>
                  ...
                  <div className="col-sm-7 col-md-6 col-lg-5">
                    <details className="brdr-0">
                      <summary className="btn btn-default text-center">
                        { ( loading || error ) ? null : data.reportApRoblem[ `header_${ locale }` ] }
                      </summary>
                        ...
                        <div className="gc-rprt-prblm">
                          <div className="gc-rprt-prblm-frm gc-rprt-prblm-tggl wb-frmvld">
                            <form onSubmit={ this.onReportSubmit } >
                              <p>{ ( loading || error ) ? null : data.reportApRoblem[ `description_${ locale }` ] }</p>
                              <fieldset>
                                <legend><span className="field-name h4"><p>{ ( loading || error ) ? null : data.reportApRoblem[ `instruction_${ locale }` ] }:</p> </span></legend>
                                { options }
                                <div className="form-group">
                                  <label htmlFor="problem-details"><p>{ ( loading || error ) ? null : data.reportApRoblem[ `details_${ locale }` ] }</p></label>
