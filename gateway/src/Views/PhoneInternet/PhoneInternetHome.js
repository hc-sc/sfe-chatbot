...
class PhoneInternetHome extends Component {

  render() {
    const { locale } = this.props.match.params;

    const queryPhoneInternetHome = gql`{
      homePages( where: { name: "phone-internet" } ) {
        header_${ locale }
        content_${ locale }
        updatedAt
        page_assets( where: { url_contains: "gw_landing_phone" } ) {
          mime
          url
        }
        sub_pages {
      ...
      <Query query={ queryPhoneInternetHome } pollInterval={ 300000 } >
        { ({ loading, error, data }) => {
          ...
          const imageLanding = !isLoaded
            ? null // TODO:
            : `http://localhost:1337${ data?.page_assets?.find( asset => /gw_landing_phone_.*\.png/.test( asset?.url ) )?.url }`;
          ...
          return (
            <Fragment>
              ...
              <CategoryCards className="col-md-6 col-sm-6" locale={ locale } cards={ data?.category_tiles } />
            </Fragment>
          );
        } }
      </Query>
...
export default PhoneInternetHome;
