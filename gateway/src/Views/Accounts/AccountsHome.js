...
class AccountsHome extends Component {
  ...
  render() {
    ...
    const queryAccountsHome = gql`{
      homePages( where: { name: "accounts" } ) {
        header_${ locale }
        content_${ locale }
        updatedAt
        page_assets( where: { url_contains: "gw_landing_account" } ) {
          mime
          url
        }
        sub_pages {
    ...
    return (
      <Query query={ queryAccountsHome } pollInterval={ 300000 } >
        { ({ loading, error, data }) => {
          ...
          const imageLanding = !isLoaded
            ? null // TODO:
            : `http://localhost:1337${ data?.page_assets.find( asset => /gw_landing_account_.*\.png/.test( asset?.url ) )?.url }`;
          ...
          return (
            <Fragment>
              ...
              <CategoryCards locale={ locale } cards={ data?.category_tiles } />
            </Fragment>
          );
        } }
      </Query>
