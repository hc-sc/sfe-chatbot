...
class SecurityHome extends Component {
  ...
  render() {
    ...
    const queryPhoneInternetHome = gql`{
      homePages( where: { name: "security" } ) {
        header_${ locale }
        content_${ locale }
        updatedAt
        page_assets( where: { url_contains: "gw_landing_security" } ) {
          mime
          url
        }
        sub_pages {
      ...
      <Query query={ queryPhoneInternetHome } pollInterval={ 300000 } >
        { ({ loading, error, data }) => {
          ...
          const imageLanding = !isLoaded
            ? null // TODO:
            : `http://localhost:1337${ data.page_assets.find( asset => /gw_landing_security_.*\.png/.test( asset?.url ) )?.url }`;
